...
NwBuilder.prototype.handleWinApp = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        var updateVersionStringPromise = rcedit(executablePath, {
            ...
            'file-version': self.options.winVersionString.FileVersion || self.options.appVersion,
            'product-version': self.options.winVersionString.ProductVersion || self.options.appVersion,
            // Winresourcer replaced to rcedit "set-icon" argument because
            // instead of winresourcer, rcedit can add ico/png icon with out any issues (tested on 2000x2000 .png icon),
            // and yes, rcedit REMOVES ALL EXISTED icons from executable file, and then add the current one.
            'icon': (self.options.winIco ? path.resolve(self.options.winIco) : undefined)
        });

        // build a promise chain
        allDone.push(updateVersionStringPromise);
    });
